...
- (void)_switchFromCameraModeAtIndex:(unsigned)from toCameraModeAtIndex:(unsigned)to
{
	%orig;
	if (autoStart)
		startDictation(YES);
...
- (void)_willChangeFromMode:(int)fromMode toMode:(int)toMode fromDevice:(int)fromDevice toDevice:(int)toDevice animated:(BOOL)animated
{
	%log;
	if (autoStart) {
